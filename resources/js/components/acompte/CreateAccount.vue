<template>
    <div class="container mx-auto p-4">
        ...
        <form @submit.prevent="submitAcompte">
            ...
            <Combobox v-model="selected" ref="combobox">
                <div class="relative mt-1">
                    <div
                        class="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
                        <ComboboxInput
                            class="w-full border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 focus:ring-0"
                            :displayValue="(person) => person ? person.name : ''" @change="query = $event.target.value"
                            @click="$refs.combobox.open()" @focus="$refs.combobox.open()" />
...
<script setup>
    ...
    import axios from 'axios'; // Assurez-vous que axios est importé si ce n'est pas déjà fait.
    ...
    const selected = ref(null); // Commencez avec null si aucune personne n'est sélectionnée par défaut.
    const query = ref('');
    ...
    const submitAcompte = async () => {
        try {
            ...
            // Ajouter product_id à la requête si disponible
            if (acompte.value.product_id) {
                payload.product_id = acompte.value.product_id;
            }

            // Assurez-vous que l'URL est correcte et pointe vers votre API Laravel
            ...
            // Gestion des erreurs
            if (error.response) {
                // Les erreurs retournées par le serveur peuvent être traitées ici
                console.log(error.response.data);
                alert('Erreur lors de la création de l\'acompte: ' + error.response.data.message);
            } else {
                // Erreurs qui ont eu lieu pendant l'envoi de la requête
