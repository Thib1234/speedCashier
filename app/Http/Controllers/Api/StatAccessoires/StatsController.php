...
namespace App\Http\Controllers\Api\StatAccessoires;
        ...
        $sales = Sale::whereDate('created_at', '>=', $startDate)->with(['client', 'products.category'])
            ...
            $filteredSales = $sales->map(function ($sale) {
                $filteredProducts = $sale->products->filter(function ($product) {
                    return $product->category && in_array($product->category->name, ['Colliers', 'Laisses', 'Paniers', 'Jouets', 'Sacs', 'Accessoires', 'Manteaux', 'Plaids']);
                });
